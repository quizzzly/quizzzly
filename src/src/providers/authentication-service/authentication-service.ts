...
import { InAppBrowser, SecureStorage  } from 'ionic-native';
...
export class AuthenticationService {
  ...
  getSecureStorage(): Promise<SecureStorage> {
  ...
  get isAuthenticated(): Promise<boolean> {
    ...
    return this.getSecureStorage().then(() => {
      return this.secureStorage.get(LOCAL_STORAGE_KEY);
    }).then(userInfo => {
  ...
  private authenticateRemote(): Promise<AuthenticatedUserInfo> {
    return new Promise((resolve, reject) => {
      ...
      let browser = new InAppBrowser(AuthenticationConfiguration.getAuthenticationUrl(randomState), "_blank", "location=no,clearsessioncache=yes,clearcache=yes");
  ...
  private saveUserInfo(userInfo: { access_token: string, user_id: string }): Promise<AuthenticatedUserInfo> {
    ...
    return this.getSecureStorage().then(() => {
      return this.secureStorage.set(LOCAL_STORAGE_KEY, serializedUserInfo);
    }).then((data) => {
  ...
  logout(): Promise<void> {
    return this.getSecureStorage().then(() => {
      return this.secureStorage.remove(LOCAL_STORAGE_KEY);
    }).catch(error => {
